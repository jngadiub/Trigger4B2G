...
def make_file_list(dirs):

 #files = []
 file_list = open('List.txt','w')
 for d in dirs:    
  for f in os.listdir(d):
   if f.find('.root') != -1: file_list.write(d+'/'+f+'\n')
 file_list.close()
...
OutputFileNames = "jet_triggers" # base of the output file name, they will be saved in res directory
ScriptName = "jet_triggers.py" # script to be used with cmsRun
...
make_file_list(['/eos/cms/store/cmst3/user/jngadiub/triggers_2018/SingleMuon/crab_SingleMuon_Run2018A-PromptReco-v1/180604_025323/0000/',
                '/eos/cms/store/cmst3/user/jngadiub/triggers_2018/SingleMuon/crab_SingleMuon_Run2018A-PromptReco-v2/180604_025902/0000/'])
...
for x in range(1, int(NumberOfJobs)+1):
   ...
   with open('job.sh', 'w') as fout:
      ...
      fout.write("cmsenv\n")
      fout.write("python "+ScriptName+" -f "+"tmp/"+str(x)+"/list.txt -o res/"+OutputFileNames+"_"+str(x)+".root\n")
